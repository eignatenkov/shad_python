...
def pa(params, model=2):
    ...
    return np.ones(size)*1./size, np.arange(params['amin'], params['amax'] + 1)


def pb(params, model=2):
    size = params['bmax'] - params['bmin'] + 1
    return np.ones(size)*1./size, np.arange(params['bmin'], params['bmax'] + 1)
...
def pc_ab(a, b, params, model=2):
    total_size = params['amax'] + params['bmax'] + 1
    if model == 1:
        c1 = binom(a, params['p1'])
        c2 = binom(b, params['p2'])

        def vector_convolve(num):
            return np.convolve(num, c2.pmf(np.arange(params['bmax']+1)))
        total_size = params['amax'] + params['bmax'] + 1
        dist = np.apply_along_axis(vector_convolve, 0,
                                   c1.pmf(np.arange(params['amax']+1).reshape(params['amax']+1, 1)))
        if isinstance(a, np.ndarray):
            return dist, np.arange(total_size)
        else:
            return dist.reshape(total_size), np.arange(total_size)
    elif model == 2:
        lmbd = a*params['p1'] + b*params['p2']
        dist = poisson(lmbd)
        support = np.arange(total_size)
        if isinstance(a, np.ndarray):
            return dist.pmf(support.reshape(total_size, 1)), support
        else:
...
def pc(params, model=2):
    ...
    bmin = params['bmin']
    bmax = params['bmax']
    support = np.arange(amax + bmax + 1)
    if model == 1:
        dist = sum(
            np.sum(pc_ab(np.arange(amin, amax + 1), b, params, model)[0], axis=1) for b in
            np.arange(bmin, bmax + 1)) / (amax - amin + 1) / (bmax - bmin + 1)
        return dist, support
...
def pb_d(d, params, model=2):
    ...
    def p_d_c(d, c, params, model):
        dist, support = pd_c(c, params, model)
        return dist[np.where(support == d)[0][0]]
    p_d_c_vect = np.array([p_d_c(d, c, params, model) for c in np.arange((d+1)/2, d+1)])
    result = []
    # calculate for every b
    for b in np.arange(params['bmin'], params['bmax']+1):
        pc_ab_sum_a = np.sum(pc_ab(np.arange(params['amin'], params['amax']+1), b, params, model)[0], axis=1)
...
def pb_ad(a, d, params, model=2):
    ...
    def p_d_c(dd, cc):
        dist, support = pd_c(cc, params, model)
        return dist[np.where(support == dd)[0][0]]
    p_d_c_vect = np.array([p_d_c(d, c) for c in np.arange((d+1)/2, d+1)])
    result = []
    for b in np.arange(params['bmin'], params['bmax']+1):
        result.append(pc_ab(a, b, params, model)[0][(d+1)/2:(d+1)].dot(p_d_c_vect))
    return np.array(result)/sum(result), np.arange(params['bmin'], params['bmax']+1)
